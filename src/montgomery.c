...
void invert(mpz_t X, mpz_t d, mpz_t x, mpz_t z, mpz_t N){
	mpz_t X0, s;
	mpz_inits(X0, d, s, NULL);
    mpz_gcdext (d , X0, s, z, N);
    mpz_mul(X0,X0,x);
    mpz_mod(X,X0,N);
    mpz_clears(X0, s, NULL);
    return;
}
...
void Suyama_param(mpz_t a24, mpz_t A, mpz_t X, mpz_t Z, unsigned int m, mpz_t mpz_4, mpz_t N){
    ...
    mpz_mul(tmp,X,v); // tmp = u^3 * v
    mpz_mul_2exp(tmp, tmp, 4); // tmp = 16 * u^3 * v
    mpz_invert(tmp, tmp, N); // tmp = 1 / (16 * u^3 * v)
    mpz_sub(a24, v, u);
    mpz_powm_ui(a24, a24, 3, N);
    mpz_mul(a24, a24, tmp); // a24 = (v-u)^3 / (16 * u^3 * v) 
    mpz_mul_ui(tmp, u, 3);
    mpz_add(tmp, tmp, v);
    mpz_mul(a24, a24, tmp);
    mpz_mod(a24, a24, N);
...
void xADD(mpz_t X, mpz_t Z, mpz_t X1, mpz_t Z1, mpz_t X2, mpz_t Z2, mpz_t X3, mpz_t Z3, mpz_t N){
    ...
    mpz_mul(a,a,tmp); 
    mpz_mod(a,a,N); // a = (X1 - Z1)*(X2 + Z2) mod N

    mpz_add(b,X1,Z1);
    mpz_sub(tmp,X2,Z2);
    mpz_mul(b,b,tmp); 
    mpz_mod(b,b,N); // b = (X1 + Z1)*(X2 - Z2) mod N
    /*
	a = ((X1-Z1)*(X2+Z2)) % N
	b = ((X1+Z1)*(X2-Z2)) % N
    */
...
/* Montgomery ladder with normalized starting point : (X,Z) = [m] (X1,1).
